...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if (!bd->pVB || bd->VertexBufferSize < draw_data->TotalVtxCount)
    {
        ...
        desc.MiscFlags = 0;
        if (bd->pd3dDevice->CreateBuffer(&desc, NULL, &bd->pVB) != S_OK)
            return;
    }
    if (!bd->pIB || bd->IndexBufferSize < draw_data->TotalIdxCount)
    {
        ...
        desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
        if (bd->pd3dDevice->CreateBuffer(&desc, NULL, &bd->pIB) != S_OK)
            return;
    }
...
bool ImGui_ImplDX11_CreateDeviceObjects()
{
...

    {
        static const char* vertexShader =
            ...
            "PS_INPUT main(VS_INPUT input) { PS_INPUT output; output.pos = mul(ProjectionMatrix, float4(input.pos.xy, 0.f, 1.f)); output.col = input.col; output.uv = input.uv; return output; }\n";
...
bool ImGui_ImplDX11_Init(ID3D11Device* device, ID3D11DeviceContext* device_context)
{
    ...
    if (device->QueryInterface(__uuidof(IDXGIDevice), (void**)&pDXGIDevice) == S_OK)
        if (pDXGIDevice->GetParent(__uuidof(IDXGIAdapter), (void**)&pDXGIAdapter) == S_OK)
            if (pDXGIAdapter->GetParent(__uuidof(IDXGIFactory), (void**)&pFactory) == S_OK)
            {
